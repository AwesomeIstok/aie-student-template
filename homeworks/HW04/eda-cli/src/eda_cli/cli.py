...
def report(
    ...
    effective_title = title if title is not None else "EDA-отчёт"
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Заголовок отчёта: `{effective_title}`\n")
        f.write(f"- Макс. число гистограмм числовых признаков: `{max_hist_columns}`\n\n")
        ...
        f.write(f"Построено не более **{max_hist_columns}** гистограмм для числовых признаков.\n")
    ...
    typer.echo(f"✅ Отчёт сгенерирован в каталоге: {out_root.resolve()}")
    typer.echo(f"- Markdown: {md_path.resolve()}")
